...
def scan_one_sykes_symbol_stateful(symbol: str, timeframe: str):
    ...
    try:
        exchange = exchange_client.get_exchange()
        state_row = database.get_symbol_state(state_key)
        is_new = state_row is None

        if is_new:
            candles = exchange_client.fetch_closed_candles(
                exchange, symbol, timeframe=timeframe, limit=SYKES_BOOTSTRAP_LIMIT
            )
            if len(candles) < 20:
                return symbol, timeframe, []
            state = sykes_strategy.new_state()
            state, _bootstrap_signals = sykes_strategy.advance_state(state, candles, rr=SYKES_RR)
            last_candle_time = candles[-1][0]
            logger.info(f'[賽克斯 {timeframe}] {symbol} 新幣種初始化完成，用了 {len(candles)} 根歷史K棒補記憶')
            result_signals = []
        else:
            state_json, last_candle_time = state_row
            state = sykes_strategy.state_from_json(state_json)
            new_candles = exchange_client.fetch_new_closed_candles(
                exchange, symbol, timeframe=timeframe, since_ms=last_candle_time
            )
            if not new_candles:
                return symbol, timeframe, []
            state, result_signals = sykes_strategy.advance_state(state, new_candles, rr=SYKES_RR)
            last_candle_time = new_candles[-1][0]

        database.save_symbol_state(
            state_key, sykes_strategy.state_to_json(state), last_candle_time, int(time.time() * 1000)
        )
        return symbol, timeframe, result_signals
    except Exception as e:
        logger.warning(f'[賽克斯 {timeframe}] 掃描 {symbol} 失敗: {e}')
        return symbol, timeframe, []
        ...
        for future in as_completed(futures):
            ...
            for signal in signals:
                ...
                database.record_signal(
                    ...
                    signal.candle_time, detected_at, strategy='xuan', timeframe=TIMEFRAME
                )
                ...
                notifier.send_telegram_message(message)
                found_count += 1
                logger.info(f'[XUAN] 新訊號: {symbol} {signal.direction} @ {signal.entry_price}')

    # ---- 賽克斯 (每個時區各自掃描全部幣種) ----
    for tf in SYKES_TIMEFRAMES:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
            futures = {pool.submit(scan_one_sykes_symbol_stateful, s, tf): s for s in symbols}
            for future in as_completed(futures):
                ...
                for signal in signals:
                    ...
                    database.record_signal(
                        symbol, signal.direction, signal.entry_price, signal.sl_price, signal.tp_price,
                        signal.candle_time, detected_at, strategy='sykes', timeframe=timeframe
                    )
                    strategy_label = f'賽克斯 {timeframe}'
                    message = notifier.format_signal_message(
                        symbol, signal.direction, signal.entry_price, signal.sl_price, signal.tp_price, timeframe,
...
def build_dashboard_data(start_ms=None, end_ms=None) -> dict:
    ...
    sykes_signals = _prepare_signals(database.get_recent_signals(limit=200, strategy='sykes'), now_ms)
    sykes_open = [s for s in sykes_signals if s['status'] == 'open']
    sykes_closed = [s for s in sykes_signals if s['status'] != 'open']
    _attach_live_pct(sykes_open)
    ...
    xuan_overall, xuan_per_symbol = database.get_stats(start_ms, end_ms, strategy='xuan')
    sykes_overall, sykes_per_symbol = database.get_stats(start_ms, end_ms, strategy='sykes')

    return {
        'xuan_open_signals': xuan_open,
        'xuan_closed_signals': xuan_closed,
        'sykes_open_signals': sykes_open,
        'sykes_closed_signals': sykes_closed,
        ...
        'sykes_overall_stats': sykes_overall,
        'sykes_per_symbol_stats': sykes_per_symbol,
        'timeframe': TIMEFRAME,
    }
